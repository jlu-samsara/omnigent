...
# Grant/revoke/reassign/admin-flag writes evict, so a same-instance revoke is
# normally visible at once; the classic cache-aside race (a reader whose DB
# snapshot predates the commit re-stores a stale positive just after the evict)
# means the revoke window is bounded by the TTL even same-instance, and is at
# most the TTL across replicas (there is no cross-replica invalidation). That
# window is LEVEL_EDIT only — the destructive stop/kill path re-gates
# LEVEL_OWNER separately. Entries are keyed by (conversation_id, user_id):
...
class SqlAlchemyPermissionStore(PermissionStore):
    ...
    def reassign_user_grants(self, from_user_id: str, to_user_id: str) -> int:
        ...
        # Grants moved between users across sessions; drop the whole cache
    ...
    def set_admin(self, user_id: str, is_admin: bool) -> None:
        ...
        # The admin flag flips access on every session for this user; drop all.
        self._invalidate_resolve_cache_all()
    ...
    def _resolve_cache_store(
        ...
        Enforces the LRU entry cap so the cache cannot grow without bound on a
        long-lived replica.
        """
        key = (conversation_id, user_id)
    ...
    def resolve_access(
        ...
        if cache_enabled and (
            ...
            self._resolve_cache_store(conversation_id, user_id, access)
        return access
